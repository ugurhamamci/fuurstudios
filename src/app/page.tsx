...
/** 905448508960 -> 0544 850 89 60 (tanınmayan biçimleri olduğu gibi bırakır) */
...
export default async function Home() {
  ...
  const team = await query(`SELECT * FROM team_members ORDER BY sort_order ASC, id ASC`);
  ...
  {/* Mobil menü bilerek <nav> dışında: .nav.scrolled üzerindeki
      backdrop-filter, içindeki position:fixed öğeler için kapsayıcı blok
      oluşturuyor ve sayfa kaydırıldığında menü nav şeridine sıkışıyordu. */}
  ...
  <section className="team section" id="team">
    <div className="container">
      ...
      <div className="team__grid stagger-children">
        {team.length === 0 && (
          <p className="body-sm" style={{gridColumn: "1 / -1", textAlign: "center"}}>
            Henüz ekip üyesi eklenmedi.
          </p>
        )}
        {team.map((member: any) => {
          const memberSocials = buildSocialLinks({ ...member, whatsappNumber: member.whatsapp });
          return (
            <div className="team__member reveal" key={member.id}>
              {member.image ? (
                <img src={member.image} alt={member.name} className="team__avatar" />
              ) : (
                <div
                  className="team__avatar team__avatar--initials"
                  style={{background: `linear-gradient(135deg, ${member.accent_color || "#C8102E"} 0%, #1a1a1a 100%)`}}
                  aria-hidden="true"
                >
                  {member.initials}
                </div>
              ...
              {member.role ? <p className="team__role">{member.role}</p> : null}
              {member.bio ? <p className="team__bio">{member.bio}</p> : null}
              <SocialLinks links={memberSocials} className="team__socials" />
            </div>
          );
        })}
      </div>
          ...
          <div className="contact__details">
            ...
            <div className="contact__detail">
              ...
              <div className="contact__detail-text">
                ...
                <span className="contact__detail-value"><a href={`tel:+${waNumber}`}>{formatPhone(waNumber)}</a></span>
              </div>
            </div>
            ...
            <div className="contact__detail">
              ...
              <div className="contact__detail-text">
                ...
                <SocialLinks links={socialLinks} style={{marginTop: "var(--space-2)"}} />
              </div>
            </div>
          </div>
        </div>
    ...
    <div className="container">
      <div className="footer__grid">
        <div className="footer__brand">
          ...
          <SocialLinks links={socialLinks} style={{marginTop: "var(--space-6)"}} />
        </div>
        ...
        <div>
          ...
          <div className="footer__links">
            {socialLinks.map(link => (
              <a key={link.key} href={link.href} target="_blank" rel="noopener noreferrer" className="footer__link">{link.label}</a>
            ))}
            <a href={`mailto:${email}`} className="footer__link" data-i18n="footer.link.email">E-posta</a>
          </div>
        </div>
      </div>
